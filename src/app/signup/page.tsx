...
export default function SignupPage() {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      // Create user in NeonDB via our API
      ...
      // If signup successful, automatically sign them in using NextAuth
      ...
      <div className="flex-1 flex items-center justify-center p-6 mt-16 md:mt-0">
        <div className="w-full max-w-md bg-[#0a0a0a] border border-white/10 p-8 rounded-2xl shadow-2xl">
          ...
          <form onSubmit={handleSubmit} className="space-y-6">
            <div>
              <label className="block text-white text-sm font-bold uppercase tracking-widest mb-2">Full Name</label>
              <input 
                type="text" 
                value={name}
          ...
          <div className="mt-8 text-center">
            <p className="text-gray-500 text-sm">
              Already have an account? <Link href="/login" className="text-white hover:text-primary cursor-pointer underline">Log in</Link>
            </p>
          </div>
        </div>
      </div>
